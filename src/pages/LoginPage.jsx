...
const LoginPage = () => {
  ...
  return (
    ...
    <div className="min-h-screen flex flex-col lg:flex-row">
      ...
      {/* ============================== */}
      {/* 2. flex-1 makes this take up exactly half the screen. The background color stretches fully! */}
      <div 
        className="hidden lg:flex flex-1 items-center justify-center p-12 xl:p-20" 
        style={{ backgroundColor: 'var(--color-primary-dark)', color: 'var(--color-surface)' }}
      >
        {/* We wrap the content in a max-w-lg div so the text doesn't stretch too wide */}
        <div className="w-full max-w-lg">
          <h4 style={{ marginBottom: 'var(--space-4)', color: 'var(--color-text-inverse)' }}>
            Doctor Time
          </h4>
          
          <h1 style={{ color: 'var(--color-text-inverse)', marginBottom: 'var(--space-4)' }}>
            Your Health In Your Hands
          </h1>
          
          <p style={{ color: 'var(--color-text-inverse)', marginBottom: 'var(--space-8)' }}>
            Book Appointments with Trusted Doctors in Doctor Time
          </p>

          <ul style={{ listStyle: 'none', padding: 0 }}>
            <li style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: 'var(--space-3)' }}>
              <span style={{ color: 'var(--color-accent)' }}><VscWorkspaceTrusted size={24} /></span> 
              Trusted doctors across specialties
            </li>
            <li style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span style={{ color: 'var(--color-accent)' }}><IoTimerSharp size={24} /></span> 
      ...
      > 
        <div className="card w-full max-w-md">
          ...
          <div style={{ textAlign: 'center', marginTop: '-32px', marginBottom: 'var(--space-4)' }}>
            <span className="text-small text-muted" style={{ backgroundColor: 'var(--color-surface)', padding: '0 var(--space-2)' }}>
              or continue with
            </span>
          </div>
          ...
          <div style={{ marginTop: 'var(--space-6)', textAlign: 'center' }}>
            <p className='text-small'>
              Need to Create an Account? <a href="#" style={{ color: 'var(--color-primary)', fontWeight: 'bold' }}>Sign up</a>
            </p>
          </div>
